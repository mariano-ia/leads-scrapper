...
export function OutreachButton({
  ...
  function handleGenerate() {
    startGen(async () => {
      ...
      if (!result.draft_id || !result.subject || !result.body) {
        toast.error("Respuesta inesperada de la generación de draft");
        return;
      }
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Vas a enviar este email a <span className="font-medium">{toEmail}</span> vía Resend.
              Reply-to apunta a tu casilla — las respuestas te llegan directo.
            </DialogDescription>
          </DialogHeader>
          <div className="text-sm space-y-2 py-2">
            <div><span className="text-muted-foreground">Asunto:</span> {editSubject}</div>
            <div className="text-xs text-muted-foreground whitespace-pre-wrap line-clamp-6 border-l-2 pl-2">{editBody}</div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setConfirmOpen(false)}>Cancelar</Button>
            <Button onClick={confirmSendResend} disabled={sending}>
              {sending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Send className="h-3 w-3" />}
              Enviar ahora
            </Button>
          </DialogFooter>
