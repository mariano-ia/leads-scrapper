    ...
    <div className="flex items-center gap-2">
      {currentMember && (
        <Avatar className="h-6 w-6">
          <AvatarFallback className="text-[10px]">{initials(currentMember.email)}</AvatarFallback>
        </Avatar>
      )}
      <select
        value={value}
        ...
        disabled={pending}
        className="text-xs rounded-md border px-2 py-1 bg-background cursor-pointer disabled:opacity-50"
      >
